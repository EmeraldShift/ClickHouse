...
--
-- Regression test for #101940 and #102320. Correctness here rests on the
-- per-CTE FutureMaterializedCTE barrier: DelayedMaterializingCTEsStep::
-- makePlansForCTEs calls future->buildInplace for each claimed CTE inside
-- the optimization pass, so any later reader -- an eager sub-pipeline fired
-- from the same plan.optimize() call, a sibling UNION branch, or the main
-- pipeline itself -- finds the storage populated by the time it reads.
