...
struct QueryPlanOptimizationSettings
{
    ...
    /// sub-pipeline (e.g. materializing a CTE through
    /// `FutureMaterializedCTE::buildInplace` inside
    /// `DelayedMaterializingCTEsStep::makePlansForCTEs`). Populated by the
    /// `ContextPtr` constructor; left null in the explicit constructor used
    /// by remote-plan deserialization paths.
