...
#define APPLY_FOR_BUILTIN_EVENTS(M) \
    ...
    M(PartMinMaxSkipsGranuleIndex, "Number of (part, minmax skip index) pairs whose granule-level evaluation was skipped because the part's partition-level minmax already proved the condition.", ValueType::Number) \
    \
